# 架构说明

## 设计目标

Daily Workbench 把“工作区”作为核心上下文。当前工作区身份、界面偏好、收件箱、任务、Markdown 笔记、滚动 7 日日程、专注会话、浏览器标签、收藏夹与受控自动化已经进入 SQLite，并由统一数据保护边界覆盖；机器相关的终端偏好也按工作区持久化，但终端会话、缓冲与 AI 会话仍只存在于 Main 运行时，不伪装成可恢复数据。业务模块不需要绕过进程隔离。

## 进程与信任边界

| 层             | 信任级别 | 职责                                                    | 明确禁止                                   |
| -------------- | -------- | ------------------------------------------------------- | ------------------------------------------ |
| Electron Main  | 高       | 生命周期、窗口、PTY、浏览器会话、AI 凭据/请求、IPC 编排 | 将任意 Electron 对象或 API key 交给页面    |
| Preload        | 高且受限 | 将窄、类型化的方法映射到 `window.workbench`             | 暴露 `ipcRenderer` 或 Node.js 通用能力     |
| React Renderer | 可信 UI  | 布局、交互、状态展示、xterm 渲染                        | 直接创建进程或读取本地文件                 |
| Remote Browser | 不可信   | 呈现用户访问的远程网页                                  | preload、Node 集成、本地 IPC、默认设备权限 |

## 浏览器

右侧浏览器由 Main 管理多个 `WebContentsView`。React 只绘制标签栏、地址栏、收藏夹、下载托盘和占位区域，通过 `ResizeObserver` 取得内容区域的 DIP 坐标，再让 Main 更新活动视图边界。

- URL 只允许 `http:` 与 `https:`。
- 标签 URL、标题、活动标签和收藏夹按工作区持久化；每个工作区最多 12 个标签、500 个收藏夹。
- 只有活动标签会按需创建远程视图。切换工作区会销毁旧工作区全部远程视图及其监听器，随后从 SQLite 恢复新工作区的标签元数据，避免隐藏页面继续运行。
- 新窗口请求默认不创建额外 Electron 窗口；安全 URL 会在同一工作区的新标签中打开。
- 摄像头、麦克风、定位、通知、USB、蓝牙等权限默认拒绝。
- 所有远程标签使用独立的 `persist:workbench-browser` session；Cookie 和登录态因此在应用内共享，不承诺按工作区隔离。
- 面板关闭时隐藏视图。窗口关闭或应用退出时先通过窄 IPC 让可信 Renderer 完成未保存笔记确认；拒绝关闭不会触碰浏览器或数据库。确认响应发送前 Renderer 会同步冻结交互，响应失败则恢复；Main 收到确认后立即禁用并隐藏窗口，再停止接收新浏览器操作，等待进行中的切换与标签元数据写入，重新捕获最终 URL/标题并显式关闭 `webContents`。数据库只会在这些步骤完成后关闭，最终窗口使用已批准的强制销毁路径，不再触发第二次 `beforeunload`。
- 每次异步读取和 mutation 都绑定请求发起时的 `workspaceId`、标签 ID 与工作区 generation。切换工作区后的迟到结果会被拒绝，A→B→A 时 Renderer 仍以单调 revision 判定新旧快照。
- Main 在工作台、终端或远程页面取得键盘焦点时统一拦截非重复、非 IME 组合态的 `Ctrl/Cmd + N`，并向可信 Renderer 发出快速记录事件；远程内容仍得不到本地 IPC。
- 浏览器快捷键同时在可信 Renderer 和远程页面入口使用同一纯函数判定，拒绝 `keyup`、自动重复、IME 组合态和冲突修饰键。

## 下载

下载生命周期只存在于 Main，并只接受当前受管浏览器标签中由用户手势触发的 `DownloadItem`。

- 每个文件都使用系统保存对话框；Renderer 不能指定默认目录、保存路径或读取最终绝对路径。
- 同时最多保留 5 个活动下载；完成、失败或取消的记录按单调序号最多保留最新 100 条，裁剪会在发布快照前完成且只存在于本次运行。
- Renderer 只能按显式 `workspaceId` 和 opaque 下载 ID 请求暂停、恢复、取消、清理或定位文件；快照只暴露经过清理的文件名、来源主机、MIME、字节数和状态。
- 定位操作只允许已完成、仍存在的绝对普通文件，拒绝符号链接，并由 Main 调用系统文件管理器显示文件；不向远程内容或 Renderer 暴露通用路径打开能力。
- 非受管 `WebContents`、缺少用户手势、超出并发上限或字段校验失败的下载会在 `will-download` 阶段直接拒绝。
- 工作区归档提交成功后，Main 会取消该工作区仍活动的下载、移除其运行时记录并释放并发槽位；数据库归档失败时不提前清理。

## 终端

终端进程只存在于 Main，Renderer 只保留 xterm.js 的显示缓冲：

1. Main 提供固定、只读的 Shell Profile 能力快照。Windows 分别探测 PowerShell 7、Windows PowerShell、CMD 与 WSL；POSIX 探测系统默认 shell、Bash、Zsh 与 PowerShell 7。每个工作区只保存固定 Profile ID，不接受自定义可执行文件、参数或环境变量。
2. 本机启动目录只能由 Main 打开 Electron 原生目录选择器取得；Renderer 只提交目标工作区和配置 revision，不能提交、拼接或重新授权路径。路径不再可用时明确阻止新会话，不静默回退到 home。
3. WSL 能力探测只调用固定的 `wsl.exe --list --quiet`，带 3 秒超时、64 KiB 输出上限且不经过 shell。输出按严格 UTF-8/UTF-16LE 解码，发行版只以绑定当前 capability revision 的 opaque ID 回传；Renderer 不能提交发行版名称。显式选择发行版时使用固定 `--distribution <已发现名称> ~`，跟随系统默认时只使用固定 `~`；两者都从 Linux home 启动，本阶段不接受任意 `--cd`。
4. 创建必须携带当前配置 revision；旧 revision、已消失的 Profile/目录/WSL 发行版都会拒绝。会话在创建时冻结可执行文件、参数、CWD 与 WSL 发行版选择，之后的设置更新不会改变或重启既有会话；退出会话的重启复用这些字段。PTY 每次 spawn 时从 Main 当前进程环境重新构造受控环境；环境不持久化，也不接受 Renderer 注入。
5. 创建、激活、重启、输入、resize、清空和关闭都显式携带 `workspaceId` 与不可预测的会话 UUID。Main 同时核对当前工作区、不可变所有者和仍存活的会话身份。
6. 每个工作区最多 8 个会话，每个窗口最多 16 个。输出事件携带工作区、会话与单调序号，单次事件最多 64 KiB；Renderer 在 surface 尚未挂载时只保留有界的 256 KiB 尾部。
7. 每个会话使用独立 xterm surface 和 PTY。切换工作区会隐藏但保留后台会话；退出状态会保留标签供用户重启，显式关闭才移除标签。
8. 工作区归档成功提交后，Main 使目标会话身份失效并尽力终止全部 PTY；数据库归档失败时不提前清理。窗口关闭、Renderer 崩溃或应用退出也会幂等地停止 WSL 探测并清理全部会话，单个 `kill` 异常不会阻断其余资源。

终端会话、输出缓冲与冻结的启动描述只存在于本次运行；工作区的 Profile、本机目录和 WSL 发行版选择进入机器本地 SQLite，但明确排除在 `.dwbx` 逻辑数据包之外。启动 PTY 时保留平台所需系统环境变量，但不会把可执行文件路径或原始启动错误返回 Renderer。

## IPC 约定

共享协议放在 `src/shared`。命令使用 request/response，持续输出使用可取消订阅事件。所有 handler 都应同时验证：

- 调用者是否为可信主界面；
- 字符串长度、枚举值和数值范围；
- URL 协议、会话 ID 和边界坐标；
- 目标对象是否仍然存活。

工作区、收件箱、任务、笔记、日程、专注、自动化、浏览器、搜索和终端请求必须显式带目标 `workspaceId`；搜索另接受固定的当前/全部活动工作区 scope。浏览器标签、收藏夹与下载操作还必须带对应 opaque ID，终端会话操作必须带对应 UUID；专注状态转换以及自动化更新或立即运行则同时绑定对应 opaque ID 与 revision。Main 不能在执行时重新读取“当前工作区”猜测这些请求的归属。

AI 是刻意收窄的例外：Renderer 不能提交 `workspaceId`，Main 在开始请求时绑定自己持有的当前活动工作区，并把该身份放入后续快照和事件。AI 上下文只接受固定枚举和 opaque 业务身份，Main 会在发请求前重新读取已绑定工作区的数据，不能接受 Renderer 拼好的上下文正文。数据导入提交只接受短期 opaque ID 与预览摘要，不能携带路径。备份恢复只能回传列表中冻结的备份 ID、原因、创建时间、字节数和 schema 版本；Main 会把这组字段作为完整的乐观并发令牌重新定位、验证并复制应用私有备份，Renderer 不能提交文件名或路径。Main 主动触发快速记录、自动化运行结果、AI 流式增量、浏览器/终端状态更新、备份状态更新与地址栏聚焦只使用固定事件通道，不暴露通用事件订阅能力。

新增功能时，应先在共享层定义输入与输出，再实现 Main handler 和 Preload 映射，最后接入 Renderer。

## 数据库

SQLite 连接、迁移、Repository、搜索、备份策略和数据可移植性都只存在于 Main。应用从 `userData` 推导固定的数据、备份和导入隔离目录，Renderer 不能提供路径、SQL、迁移版本或新的备份原因。数据库管理只暴露类型化状态、受控备份/策略操作、应用备份恢复与逻辑数据包流程；工作区、收件箱、任务、笔记、日程与浏览器持久数据仍通过各自的窄业务接口访问。

数据库服务是应用级单例：首个窗口创建前打开；macOS 关闭单个窗口时保持连接，正常受控退出则先停止窗口运行时和应用级调度器，把仍在运行的专注会话原子暂停，再排空数据库操作队列并关闭。Windows 注销/关机和强制终止不能依赖退出事件，因此重启时会用持久化绝对截止时间对账；SQLite 事务与 WAL 继续负责崩溃恢复，未完成的临时备份不会被列为可用备份。连接启用 foreign keys、WAL、busy timeout、defensive mode 与 `trusted_schema=OFF`，禁止加载扩展。

迁移通过构建期 raw import 嵌入 Main bundle，按连续版本在事务中执行，并把名称与 SHA-256 校验和写入历史表。已应用迁移发生漂移或数据库版本高于当前代码时拒绝启动。已有数据库升级前会先创建经过完整性验证的一致性备份；备份失败时不会继续迁移。

v7 追加外部内容 FTS5 索引和应用级备份策略/运行状态；v8 追加每工作区一行的机器本地终端偏好及 revision 并发保护；v9 追加默认停用的自动化定义、运行状态和不可变 occurrence 账本；v10 追加固定 25 分钟的专注会话、全局未结束唯一性与不可变历史；v11 追加归档工作区恢复 revision、CAS、归档元数据保护与恢复时的自动化全局上限。逻辑导入不复用外部数据库表结构，而是在隔离区验证受支持的版本化数据包并构建 schema v11 的 staging 数据库：source schema v10/v11 继续共用 `.dwbx` v3，可携带专注历史和自动化定义，但 staging 会把未结束专注与全部自动化都强制暂停；v9 `.dwbx` v2 和 v7/v8 `.dwbx` v1 继续接受。所有来源都会先在空库应用完整 v1–v11 迁移再导入业务记录，机器终端配置不会进入 staging。

应用备份恢复使用相同的隔离和替换底座，但不接受外部 SQLite：Main 先按冻结的五字段元数据令牌重新验证应用生成的备份，再把源文件复制到唯一隔离区并锁定首次 SHA-256；全运行时冻结后的第二次复制必须与该摘要相同，避免用户确认期间发生目标漂移。v0–v10 副本通过正常数据库启动路径升级到 v11，源备份保持逐字不变；因为规范源本身已经是受控保留的应用备份，隔离副本迁移不会在深层临时目录中再生成一份冗余的 `pre-migration` 快照。staging 会保留当前数据库的自动备份策略和调度运行状态，停用全部自动化；仍在运行且已到期的 Focus 结算为完成，否则暂停，并把所有工作区的浏览器/终端面板偏好置为关闭。原始浏览器视图、PTY、下载和 AI 请求在全运行时冻结及进程重启中释放，Cookie、登录态、API key 和文件系统下载等数据库外状态不会被回滚。

导入或备份恢复在关闭旧连接前都必须生成并重新验证当前数据库的安全备份；随后由同一 v1 crash marker 驱动重启阶段的原子替换和失败回滚。成功安装会把旧数据库原子发布为 `pre-import-recovery-<id>.sqlite3`，这是沿用既有协议的兼容文件名，语义是任意成功替换后不进入普通备份列表的最后恢复副本。备份管理可以列出应用识别的完整历史并按严格令牌恢复，但仍不接受任意路径、外部 SQLite 或删除请求。详细规则见[数据库与迁移](DATABASE.md)。

## 定时与手动自动化

自动化 Controller 是应用级单例，不绑定窗口或当前工作区。它在数据库打开后启动，系统从睡眠恢复时重新评估；数据替换和应用受控退出必须先停止调度并等待正在进行的 single-flight 评估，再关闭数据库。关闭 macOS 窗口或 Renderer 崩溃不会让应用级调度器伪装成已停止。

- 定义只允许每日/每周本地时间，以及固定的“创建今日任务”或“创建 Markdown 笔记”；创建时默认停用，动作类型创建后不可更改。
- 既有 `automation:create` 在同一个 Main 事务中返回事务后的完整 `automationSnapshot` 与实际插入的 opaque `createdAutomationId`；这只扩展返回契约，不新增 IPC 通道、schema 或 migration。Renderer 创建成功后仍留在发起页面并显示“默认停用”的回执；只有用户显式打开时才 fresh-read 当前工作区规则、按该 ID 唯一匹配，并用权威快照中的当前 revision 打开编辑窗口。目标缺失、重复 ID、切页、较新创建、工作区 A→B→A 或迟到请求都失败关闭，不按名称、计划或列表位置回退。
- 若 Main 已提交创建但 Renderer 在有界重读后仍无法提交权威快照，创建窗口会关闭并显示不依赖页面状态栏的独立警告，明确要求刷新且不要重复创建；此路径不发布未经确认的成功回执。
- 用户可以对活动且未归档的定义显式请求立即运行，无论该定义当前启用或停用。可信 Renderer 必须先完整展示已保存动作并获得确认，随后只提交 `{workspaceId, automationId, expectedRevision}`；Main 在数据库事务中重新读取工作区与定义、核对 revision，并只执行持久化的动作，不能接受 Renderer 拼出的任务、笔记正文或运行时间。
- 手动运行成功只原子创建一条任务或笔记并返回输出类型与 opaque ID；它不改变 `enabled`、`effective_at`、定义 revision、`automation_run_state`、`automation_occurrences` 或下一次计划时间，也不会消费或伪造一个计划 occurrence。相邻的定时 occurrence 仍按原计划独立判定。
- 同一工作区的所有显式手动运行共享 single-flight 身份，从调用 Main、输出对账到恢复状态发布完成前都不能启动第二条规则；不同工作区的在途身份彼此隔离。Main 成功返回后，Renderer 先检查对应任务或笔记的已提交快照，再最多做两次权威 fresh-read，只接受当前数据库中与返回种类一致且 opaque 输出 ID 唯一匹配的对象。每次读取、commit 与发布前都重新核对原运行身份；不能提交精确快照时不发布普通成功，而是跨同工作区切页保留“已运行、不要重复运行”的精确恢复状态与单飞重新读取入口。工作区 A→B→A 的旧 activation 不能发布普通成功，仍在途的已提交结果只会失败关闭为保守警告；真正获批的数据替换、较新恢复和迟到结果也不能覆盖新状态，取消或失败的数据替换不会提前清除仍属于当前数据库的恢复身份。
- Renderer 只为每个工作区最新一次已确认的显式手动运行保留成功入口，不自动跳转。用户点击后仍必须重新读取对应任务或笔记快照，按 Main 返回的精确 opaque ID 复核目标，并在工作区、页面或反馈身份变化时淘汰迟到导航；计划运行事件不携带 ID，也不会生成或恢复入口。
- Renderer 不得提交 cron、可执行文件、参数、环境变量、路径、URL、Webhook、通知、AI 提示或任意脚本。当前也没有条件触发、多步骤流程、Shell/终端/WSL、外部网络、系统通知或 AI 动作。
- 每个工作区最多启用 25 条，全部活动工作区最多保留 100 条活动定义。启停与编辑使用 revision，并在数据库与 Service 两层校验。
- 调度唤醒间隔最长 60 秒，同一时刻只运行一个评估。应用关闭期间不执行也不累计队列；再次启动或恢复时，每条规则只考虑最近一次错过的 occurrence。
- 计划执行的输出行、不可变 occurrence 账本和成功水位在同一事务提交，以 `(automation_id, occurrence_date)` 防止重复。失败只记录有界错误状态并退避，不留下半个任务、笔记或成功账本。
- 完整 SQLite 备份保留定义、启停状态、运行状态和 occurrence 账本；`.dwbx` v2/v3 仅携带定义，导入后全部暂停且没有历史运行状态。

## 专注会话

专注 Controller 与自动化一样是应用级单例，不绑定窗口或当前工作区。固定时长为 1500 秒；Renderer 不能提交时长、截止时间、状态、完成时间或本地日期，只能在明确工作区中开始自由专注或关联该工作区今天的一项未完成任务，并以 session ID 与 revision 请求暂停、继续或取消。

- 数据库全局最多一条 `running`/`paused` 会话。它保留原工作区身份，切换工作区不会转移或隐式取消；其他工作区只显示归属和显式切换入口。
- `running` 使用 Main 按实际 wall clock 写入的绝对 `deadline_at`，并以进程内单调时钟维护不会回增的剩余上限。Controller 最长 60 秒重新评估并把更小的上限 checkpoint 到 SQLite；系统时间回拨不会延长原 deadline，系统恢复和应用重启也会立即对账。审计时间戳保持逻辑单调，因此在回拨后可以晚于独立的倒计时 deadline。
- 正常退出和数据替换会在关闭数据库前把运行会话暂停。进程异常终止不能依赖退出回调，因此下次启动按持久截止时间完成或恢复倒计时，不使用 Renderer `setInterval` 作为权威状态。
- 状态机只允许 `running ↔ paused`、`running → completed/cancelled` 和 `paused → cancelled`；内部 checkpoint 只能在 deadline 逐字不变时严格减少 running 的剩余值。终态不可改写也不可永久删除。归档所属工作区会在同一数据库事务中取消未结束会话。
- Today 与任务页只为当前 Main 任务快照中计划到今天且未完成的任务提供专注入口；入口只打开同一个固定 25 分钟确认窗口并预选 opaque 任务 ID，不直接写入数据库。预选任务在确认前完成、改期、消失或跨午夜失效时，Renderer 保留原选择身份并阻止提交，不能静默降级为自由专注；用户必须显式改选仍有效的今日任务或自由专注。
- 可选任务必须属于同一活动工作区、计划到 Main 当前本地日期且未完成；Main 在事务中重新读取任务并拒绝旧窗口或竞态，会话一旦创建，工作区、任务和 `local_date` 都不可改写。Today 的完成轮次按持久 `local_date` 计算。
- Focus 快照在同一读事务中返回目标工作区/当天按 SQLite 持久插入顺序选出的最近一条终态摘要。会话只追加、终态不可改且全局最多一个未结束会话，所以该顺序不受跨会话墙上时钟回拨影响；摘要同时覆盖 `completed` 与 `cancelled`，较新的取消会话会压住更早的完成提示。前台完成、睡眠恢复、切换工作区后返回和重启对账都读取同一持久身份，不依赖 Renderer 曾见过 `running → null`。
- 最近终态只有在 `completed` 且含精确 task ID 时才生成 Today 收尾卡。它绝不自动修改任务；用户点击后先重新读取任务快照，复核工作区、Main 日期、完整 completion identity 与 opaque task ID，再调用既有任务状态 mutation。已完成视为幂等成功，缺失时明确失败，不按标题或列表位置回退。工作区 A→B→A、跨午夜、新终态、重复点击或迟到响应都由 activation identity、generation 与 single-flight gate 淘汰。
- `.dwbx` v3 保留已完成历史；导出时，尚未到期的 running 会话按绝对截止时间降为 paused，已经到期但尚未持久对账的会话只在包中投影为 completed，源库读取保持无副作用。导入只接受 paused/completed，取消记录不进入逻辑包。完整 SQLite 备份则保留所有状态。

## 工作区 AI 助手

AI Controller 与 provider 只存在于 Main。API key 会由用户短暂输入可信 Renderer 的设置表单，再通过窄配置调用提交；提交后 Renderer 清空输入，Main 不提供回读。Renderer 只能读取不含凭据的可用状态、设置或删除 API key、开始一次受限生成、取消当前生成，以及订阅带 request/workspace 身份的流式状态；“已配置”只是布尔状态。

- Provider 固定调用 OpenAI Responses HTTPS endpoint 和 `gpt-5.6`，每次请求都显式发送 `stream: true`、`store: false`、`tools: []`。除窄的一次性凭据配置调用外，Renderer 不能提交或读取 API key；也不能提交 endpoint、model、headers、system prompt、tool、URL、文件、路径或网络代理。
- 网络请求只从 Main 发出；可信 Renderer 的 CSP 不开放 OpenAI `connect-src`，不可信远程浏览器没有 preload 或 AI IPC。
- API key 先由 Main 通过 Electron `safeStorage` 加密，再原子写入权限限制为当前用户的机器本地文件。它不进入 SQLite、备份、日志或 `.dwbx`。Linux `safeStorage` backend 为 `basic_text` 时拒绝设置与使用 key，不降级到可逆明文。
- 上下文默认只有用户提示词。用户可显式选择当前工作区的 Today、若干未完成任务 ID，或一篇已保存笔记的 `{noteId, revision}`；三类范围互斥。Today 上下文只计入 `todayDate` 当天的未完成任务与日程，不会因日程快照扩展为 7 天而携带未来计划。Main 在开始请求前重新读取并核对当前活动工作区、归档状态、任务状态、笔记 revision 与固定数量/字符上限。浏览器页面/Cookie/下载、终端/PTY/CWD/环境/WSL、收件箱、其他笔记、搜索结果、自动化和本地文件都不会隐式发送。
- 应用同一时间最多一个 active request；请求和 SSE 事件都有大小、类型、顺序与超时上限。只接受文本/拒答增量和最终完成事件，未知输出项、工具调用、畸形 JSON、非 SSE、HTTP 错误、断流和超限都以有界错误结束，不能把 provider 原始响应或 key 回传 Renderer。
- 用户取消、工作区切换、工作区归档提交、数据替换和应用退出会先中止相关 fetch 并使 generation 身份失效。取消后的迟到 chunk 不能进入新工作区或覆盖后续请求；关闭数据库不需要等待远端自然完成。
- 模型输出始终按不可信 Markdown 处理，复用既有受控预览，不执行原始 HTML，也不会自动打开模型生成的链接。
- 提示、回答和会话只存在于本次运行，不进入 SQLite 或逻辑数据包。只有完整成功后由用户显式触发的“保存为笔记”才调用既有 Note Service 新建普通笔记；既有 `note:create` 响应同时返回完整快照和 Main 在事务中实际插入的 `createdNoteId`，Renderer 不再通过快照差集猜测创建结果。保存成功仍停留在 AI 页；只有用户再点击“打开笔记”，Renderer 才 fresh-read 当前工作区快照、按该 opaque ID 精确复核并在快照真正提交后导航，目标缺失时不会按标题、时间或列表位置回退。模型输出本身没有数据库、终端、浏览器、自动化或其他工具权限。

`store: false` 关闭 Responses 对象存储，但不代表请求没有离开设备，也不替代组织级保留策略。界面必须在发送前披露具体上下文；传输和服务端处理仍适用 [OpenAI API 数据控制](https://developers.openai.com/api/docs/guides/your-data)。API 使用通过用户自己的 OpenAI Platform 项目独立计费，不属于 ChatGPT 订阅。

## 全局搜索

搜索 Service 与业务 Service 共享数据库单例和 FIFO 操作队列。v7 的外部内容 FTS5 索引覆盖收件箱正文、任务标题、笔记标题/正文、日程标题、浏览器标签与收藏夹标题/URL；结果始终回连原表检查活动工作区和归档状态。日程查询在数据库 FIFO 操作真正开始后计算当天与第 7 天边界，只返回当前滚动窗口，避免排队跨午夜时沿用旧日期。

- 查询在共享层做良构 Unicode、NFKC、长度与不可见字符校验，FTS 查询由 Main 构造为 phrase，Renderer 不能提交运算符。
- 两字符查询走参数化、转义后的 `LIKE`；三字符以上使用 trigram FTS，兼顾短中文查询与较长正文。
- scope 只允许当前工作区或全部活动工作区。排序先考虑精确/前缀/包含匹配和当前工作区，再使用类别、相关性、更新时间和稳定 ID。
- 每类及总结果都有固定上限，返回值显式标记截断类别；搜索结果只含导航所需的 opaque 身份、标题、受限摘要和工作区名称。

## 备份与数据可移植性

自动备份调度是应用级单例，不跟随工作区切换。策略默认关闭，只允许每日/每周、本地分钟、可选星期和 1–90 个保留数量；策略更新使用 revision。调度器为 single-flight，并持久记录最近尝试、成功 bucket、失败码和连续失败次数。创建与保留清理都完成后才记录当前 bucket 成功；任一步失败都会按 5 分钟起、最长 6 小时的指数退避重试完整备份轮次。保留数量在执行删除的同一个数据库队列操作中重新读取，不能用已被新设置取代的旧值删除文件。保留清理只作用于 `scheduled` 备份，手动、迁移前和导入前备份不会被自动清理。

手动备份调用分成两个发布阶段。`database:create-backup` 拒绝前仍是普通可重试失败；一旦 Main 返回完整 `DatabaseBackupInfo`，文件已经耐久发布，Renderer 就不能把之后的列表读取或 React 发布失败伪装成“创建失败”。Controller 冻结六字段手动身份，先检查 ref-backed committed snapshot，再最多准备两次带 generation 的权威读取；只有唯一 exact ID 的全部元数据匹配且快照 commit 成功，才发布普通成功。无法确认时，始终挂载的 App 层保留独立警告并同时锁住设置页与命令面板的手动入口；恢复读取共用 Controller task 单飞且绝不重发创建 IPC。已确认身份继续保护快照，避免同 policy revision 的迟到事件隐藏新备份；保护与警告只在备份恢复返回 `restarting` 或导入提交成功后失效，取消、失败和关闭审批本身都不清除。

逻辑数据包是版本化的规范 JSON 行流，不是 SQLite 文件，也不包含 FTS 索引、自动化运行状态/occurrence、终端/浏览器/AI 会话运行时、API key 或机器相关的终端 Profile、路径与 WSL 选择。source schema v10 和 v11 都使用 `.dwbx` v3，携带自动化定义以及未取消的专注记录；v11 没有提升包格式，也不导出内部 workspace recovery revision。导入时所有自动化和未结束专注都强制暂停，并始终构建 schema v11 staging。v9 `.dwbx` v2 与 v7/v8 legacy v1 包仍可导入。Manifest 绑定源版本、记录计数和 SHA-256，解析器限制大小、记录数、字段集合和引用图。导出在文件对话框返回后立即冻结目标的“缺失或既有文件身份”，写入同目录唯一临时文件并 `fsync`；最终重命名前目标若被创建、替换或修改就拒绝覆盖，发布后再逐字回读并同步父目录。

导入预览在受控隔离目录中生成 schema v11 staging 数据库，并以有时限的 opaque ID/摘要绑定确认；替换 marker 只记录受控文件名、摘要、阶段和导入前备份 ID，使启动恢复能够决定继续安装、提交或回滚。替换过程中每次文件发布和 marker 更新都同步文件与目录；缺失 marker 时只对“主数据库缺失且恰有一个受控 rollback”做自动恢复。

真正首装会在创建 `backups/` 和打开 SQLite 之前，先以文件与目录 `fsync` 发布固定的 `database-initializing-v1` 意图；只有这个有效意图可恢复首装崩溃留下的缺失或 0 字节主库。完整初始化后先耐久发布 `database-initialized-v1` 哨兵，再严格复核并删除意图、同步目录。哨兵存在、升级前版本留下的真实 `backups/` 目录存在、普通备份存在、retained recovery 存在或 rollback 状态不唯一时，主库缺失都不会被当成首装；意图不能绕过哨兵或已识别备份，预先存在但没有意图的 0 字节主库也直接拒绝。仅预建但完全空的应用数据目录仍可首次初始化。对既有数据库，Main 在 SQLite 打开前后复核同一文件身份、大小和修改时间，再允许运行 PRAGMA 或迁移。任何数据库/rollback 并存、多个孤儿、主数据库缺失但仅有 retained recovery、主库缺失而普通备份、旧版备份目录或哨兵仍存在，或主库路径被截断/替换的情况都会停止并保留现场，不创建一个空库伪装成首装。

数据目录在 POSIX 上限制为当前用户访问，启动时拒绝符号链接或错误文件类型；导入、导出和恢复也会在异步边界复核文件身份与内容摘要。这些机制覆盖崩溃、断电、意外同步和普通文件替换，但不把已能以同一账户持续改写应用私有目录的进程视为隔离对手：在最终系统调用之间主动交换整个目录 inode 需要平台专用的目录句柄/`openat` 方案，当前不作跨平台承诺。

## 工作区

工作区 Repository 和 Service 只使用数据库单例的同一连接，并复用关闭时等待的 FIFO 操作队列。Renderer 每次取得一个原子快照：活动工作区列表、当前工作区 ID 与当前偏好，不分别读取可能互相错位的状态。

- 新库在 Main 中生成一个默认工作区；UUID、时间戳和默认值不由 Renderer 提供。
- 创建会同时写入工作区、默认偏好和当前选择；任何一步失败都会整体回滚。
- 归档是软归档。不能归档最后一个活动工作区；归档当前项时会在同一事务中先切换到确定性的备用项。
- v11 的归档快照只暴露受控工作区元数据、归档时间与 Main 维护的 recovery revision。恢复必须提交快照中的 `workspaceId`、`expectedRevision` 和显式名称；revision CAS 在同一事务中防止迟到或重复恢复。
- 归档名称可能已被新的活动工作区占用。Main 不会静默生成后缀，Renderer 必须让用户显式改名后重试；成功恢复只刷新活动/归档快照，不写当前工作区选择，因此不会自动切换。
- 当前页面、主题、侧栏、浏览器/终端面板开关和尺寸按工作区保存。
- Renderer 对偏好使用乐观显示，并为每个 `workspaceId + field` 维护 epoch、单调 sequence、最新期望值和失败所有权。写入成功或失败只有精确匹配当前字段的 epoch/sequence 时才能清 dirty 或发布保存错误；pending cleanup 则绑定发起它的请求 epoch。值相同不代表请求相同，所以字段值 A→B→A 后，最早 A 的迟到完成不能误清最后 A。
- “重试保存”只冻结并重发当前 epoch 中仍 dirty/失败字段的最新值，当前保存状态也不吸收已被更新写入取代的旧失败。读取重试或已确认提交的数据替换会同步推进 epoch，取消或失败的数据替换不会；旧 epoch 的 success/failure/cleanup 均不能改变新 epoch 的 dirty、错误或 pending。工作区 mutation 返回的完整快照仍按既有 workspace/revision 规则重基，不能覆盖其间已提交或待重试的偏好。
- 逐字段身份只用于 Renderer 的工作区偏好 patch，不扩展为工作区创建、重命名、激活、归档或恢复的提交后对账。Main 的偏好 Service、现有 IPC 输入/返回和 SQLite schema 均保持不变。
- 浏览器标签元数据、活动标签和收藏夹按工作区隔离；切换时销毁旧工作区远程视图，但共享的浏览器 session/Cookie/登录态仍为应用级上下文。下载记录只存在于 Main 运行时并按工作区过滤，归档工作区时会被清理。
- 终端进程、冻结启动描述与缓冲区只存在于 Main 运行时并绑定创建时的工作区；Profile、本机 CWD 与 WSL 选择则按工作区保存为机器本地偏好。切换会保留后台 PTY，归档成功会清理目标工作区，其他工作区不受影响。
- 归档时已停用的自动化保持停用，恢复前还会同时在 Service 与数据库校验全部活动工作区最多 100 条未归档定义。归档时已取消的未结束 Focus 会话保持终态；浏览器页面、下载、PTY 和 AI 请求等已释放运行时也不会因恢复而重建。
- 永久删除仍不暴露；归档与恢复前后的持久数据都会进入一致性备份和受支持的 `.dwbx` 逻辑包。

## 快速记录与收件箱

收件箱与工作区共用同一个 SQLite 连接和 FIFO 操作队列。Renderer 取得包含 `workspaceId` 的完整活动条目快照，并按请求序号丢弃旧工作区或旧请求的迟到响应。

- 条目 ID、时间和撤销令牌只由 Main 生成；Renderer 只能提交目标工作区、正文和固定分类。
- 既有 `inbox:create` 响应携带同一事务后的完整收件箱快照，以及 Main 实际插入的 opaque `createdEntryId`；这只扩展返回契约，不新增 IPC 通道、schema 或 migration。
- Renderer 先尝试提交事务快照；若请求序号已被较新的收件箱读取取代，则最多做两次权威 fresh-read，并在每次异步边界后复核 activation identity。所有恢复都要求 `createdEntryId` 在同一工作区恰好匹配一条记录，不按正文、分类、时间或列表位置猜测。
- Today 与全局快速记录同步成功后都留在原页并发布统一回执。用户显式打开时才再次 fresh-read 当前工作区快照、只按 `createdEntryId` 精确匹配并在快照 commit 后导航；目标缺失、已归档或状态变化时不会回退。若 Main 已创建记录但两次读取后仍不能提交权威快照，创建面会关闭并发布独立警告，明确要求重新读取且不要重复添加；只有重新读取精确确认并提交后才升级为成功回执。真正的 `inbox:create` IPC 失败仍保留原输入和错误所有权，允许显式重试。
- 正文只去除首尾空白并验证良构 Unicode、控制字符和长度，不做 NFKC 改写，避免破坏 URL、代码和技术文本。
- 分类固定为未分类、任务线索、笔记和链接。“任务线索”不代表已经创建任务。
- 归档只写入 `archived_at`。成功提交后，Main 在内存中保存 15 秒有效的一次性 opaque 撤销令牌；后端有效期使用不受系统时钟回拨影响的单调时钟，令牌不能跨工作区、重复或过期使用，也不会在重启后恢复。
- Renderer 冻结归档发起时的精确条目身份、Main 返回的 opaque token 与绝对 `undoExpiresAt`。通知只使用该绝对截止时间换算剩余的单调时长，不会在 IPC 返回、重新渲染或恢复读取后重新补足 15 秒；已经到期的 token 不会继续显示为可撤销。
- `inbox:archive` 和 `inbox:undo-archive` 的 Main 成功都是不可重放的提交边界。Renderer 依次检查响应快照、ref-backed 最新已提交快照，并在必要时最多准备两轮权威 fresh-read；归档要求 exact ID 从当前工作区活动快照消失，撤销则要求响应中的 exact 条目唯一恢复，且对应快照必须真正 commit。
- Main 已提交而上述事实仍不能发布时，App 会跨页面保留精确条目与 token 对应的“已归档/已撤销，请勿重复操作”警告。重新读取只继续同一对账，绝不再次调用归档或消费 token；恢复完成前会阻止同一工作区的竞争归档、撤销和相关条目操作。
- 每个工作区的数据 epoch 统一约束归档链路，每个 opaque token 的撤销也保持 single-flight。迟到 response/read/commit/finally、工作区 A→B→A 或旧 epoch 不能结束或覆盖新 intent；取消或失败的数据恢复/导入保留原数据库对应警告，只有数据库替换真正提交后才使旧 epoch、通知与 token 失效。
- 多条已确认归档通知仍可以并存；切换工作区后旧响应不会写入新工作区界面。上述对账只收紧 Renderer 发布与恢复语义，不改变 Main Service、preload/IPC 合约、schema v11、migration 或 `.dwbx` v3。
- 工作区归档不会删除或改写其收件箱数据，但该工作区中的条目会变为只读，继续进入一致性备份。
- 转换为任务或笔记时，在单一事务中创建目标、写入唯一来源关系并归档收件箱条目；失败时整体回滚。已转换或已归档条目不能重复转换。
- 既有转换响应分别携带 Main 在该事务中实际插入的 `createdTaskId` 或 `createdNoteId`，以及事务后的任务/笔记与收件箱快照；这只扩展已有通道的返回契约，不新增 IPC 通道、schema 或 migration。
- Renderer 把目标与来源归档视为一个不可拆分的提交结果。事务响应若只提交了一侧，会最多再做两轮权威读取；目标必须在当前工作区按 created ID 唯一存在且仍绑定原始 `sourceInboxEntryId`，来源条目也必须已经归档。任一侧无法确认时都不会发布普通成功反馈，而是保留 exact ID 与来源 ID 的独立警告，要求重新读取且不要重复转换。
- 同一工作区的所有转任务与转笔记共用单飞身份，任一转换完成双侧对账前不会让第二个来源进入 Main。转笔记确认或警告存在时还会阻止竞争笔记写入、工作区切换和跨工作区搜索，恢复读取期间重新持有笔记协调器；转任务警告仍可由工作区切换或 A→B→A 淘汰。真正获批的数据恢复/导入以及每个 read/apply 之后的 generation 变化会淘汰对应旧恢复或发布；用户取消备份恢复或导入提交失败时数据库未替换，现有警告与 exact ID 也不会提前失效。转任务警告在模态关闭后发布；直接转笔记只会在来源按钮移除导致焦点落到页面本身时接管恢复动作焦点。
- 转换成功后 Renderer 仍停留在 Inbox，并显示显式打开入口。用户点击后必须 fresh-read 对应快照，同时精确匹配创建 ID 与原始 `sourceInboxEntryId`，并在快照 commit 成功后才导航；不会仅凭标题、时间、来源关系或列表位置猜测目标。
- 转换反馈与导航 intent 都绑定当前 Inbox 页面、反馈身份和工作区 activation 对象。切页、较新转换、工作区切换、A→B→A 或迟到 read/commit/finally 会淘汰旧意图；打开失败仍留在 Inbox，不会回退到其他任务或笔记。

## 任务与 Today

任务与工作区、收件箱共用同一个 SQLite 连接和 FIFO 操作队列。Renderer 每次取得带 `workspaceId`、Main 计算的本地民用日期 `todayDate` 和固定 7 个 `{token, date}` 项的完整任务快照；创建、移动和转换只接受 `day-0` 至 `day-6` 或 `none` 计划意图，不允许页面提交任意持久化日期、完成时间或来源关系。

- 任务 ID、时间戳、`plannedFor`、`completedAt` 和收件箱来源只由 Main 生成或推进。
- 既有 `task:create` 在同一个 Main 事务中创建任务，并返回事务后的完整 `taskSnapshot` 与实际插入的 opaque `createdTaskId`；这只扩展返回契约，不新增 IPC 通道、schema 或 migration，也不改变 schema v11 或 `.dwbx` v3。
- Today、Tasks 与命令面板手动创建成功后都留在发起页面并显示回执。只有用户显式打开时才 fresh-read 当前工作区任务、按 `createdTaskId` 精确匹配，并在权威快照 commit 后进入 Tasks 的编辑界面；目标缺失会留在原页报错，工作区、页面、反馈或较新请求变化则失败关闭，不会按标题、计划日期或列表位置回退。若 Main 已提交创建但 Renderer 仍无法提交权威快照，创建框会关闭并明确提示刷新且不要重复创建，同时不发布未经确认的成功回执。
- 状态固定为 `todo`、`in_progress`、`completed`；完成时间必须与状态一致。
- `plannedFor` 是本地民用 `YYYY-MM-DD` 或 `null`。Today 的今日清单只展示计划到 `todayDate` 的任务；滚动计划与完整任务页使用同一真实快照展示接下来 7 天，窗口外历史日期仍可读取但不能由 Renderer 伪造成新计划。
- Today 从同一完整快照中独立派生 `plannedFor < todayDate` 的未完成遗留项，按旧计划日期、创建时间与 opaque ID 稳定排序并有界预览。它们不计入今日进度、AI Today 上下文或专注候选，也不会自动顺延；用户只能逐项完成、提交 Main 签发的 `day-0…day-6`，或提交 `none` 移出计划。
- 标题可重命名，任务可在当前 7 日窗口内移动或移出计划。所有 mutation 返回新的完整快照；Renderer 只有在权威快照实际提交到当前 activation 后才报告成功，并按工作区激活对象、民用日期、请求序号与 task ID 淘汰 A→B→A、跨午夜、重复操作和迟到响应。
- `sourceInboxEntryId` 为可空且唯一的来源关系。显式转换会原子创建任务并归档来源收件箱条目，响应中的 `createdTaskId` 是该事务实际插入的任务；迁移到 v4 时不会把旧 `task` 分类自动转换为任务。
- 归档工作区保留任务和来源关系以进入一致性备份，但 Service 与数据库触发器都拒绝继续修改。
- Today 中的今日任务、进度、今日日程和 7 日计划都使用真实 SQLite 数据；未来任务不会进入专注任务选择。

## Markdown 笔记

笔记与既有业务模块共用数据库单例和 FIFO 操作队列。列表快照显式携带 `workspaceId`，所有创建、编辑、归档和收件箱转换都绑定请求发起时的目标工作区。

- 笔记 ID、时间戳、revision 和收件箱来源只由 Main 生成或推进。
- 创建响应携带 Main 实际插入的 `createdNoteId` 与同一事务后的完整笔记快照；Renderer 只按该 ID 取得新建对象，不能根据标题、正文、时间或调用前后的 ID 差集猜测。Renderer 的笔记快照、加载、错误和 pending 操作同时绑定每次工作区 activation 对象，A→B→A 后旧 A 的迟到成功、失败或 cleanup 不能进入新的 A 页面。
- 手动创建只有在事务快照或最多两次权威 fresh-read 中唯一找到 `createdNoteId`，且对应快照真正提交到当前 activation 后，才清除草稿并把编辑器交接到新笔记。Main 已提交但 Renderer 仍无法同步时，App 按 workspace/page generation activation 保存精确 ID 与已提交内容；Notes 组件即使因同工作区切页而卸载，返回后也会重建不可再次保存的内容和独立重新读取警告。创建 pending 或该警告存在时会阻止工作区切换、新建或归档当前工作区以及跨工作区搜索；只有精确恢复或真正获批并提交的数据替换才使旧状态失效。真正的 IPC 创建失败才保留可编辑、可重试的草稿。缺失 selection 不会静默回退到列表第一篇笔记。
- `note:update` 成功响应必须在当前工作区唯一匹配原 opaque ID、规范化标题/正文、预期 revision、`sourceInboxEntryId` 与 `createdAt`；实质更新的 `updatedAt` 不得回拨，无内容变化则 revision 与 `updatedAt` 必须保持不变。`note:archive` 只有在同一工作区已提交的活动快照中找不到该 ID 时才确认。Renderer 会依次尝试事务响应、最新已提交 ref、最多两次 fresh-read 和最终 ref 复查，并在每次 await 与 commit 前后验证原 intent 仍有效；`commit=false` 不能清草稿或 selection。
- Main 已成功但上述确认仍失败时，App 持有精确 update/archive intent、事务快照和只读展示内容；Notes 重挂载仍显示“已保存/已归档，等待同步”，且恢复 busy/error 不落回组件局部状态。保存或归档按钮失焦时才把焦点交给“重新读取”；更新恢复后进入精确笔记，归档恢复后回到 Notes 标题。整个工作区在手动创建、编辑、归档、收件箱转笔记、AI 保存与立即运行笔记动作的写入/对账周期内保持 single-flight；精确笔记警告未恢复时继续阻断竞争写入、工作区变更和跨工作区搜索，恢复读取重新持有同一协调器。
- 数据恢复或导入只有在 Main 真正批准并提交替换后才使 note mutation coordinator 与警告失效；用户取消、原生确认取消或提交失败都保留原数据库对应的精确恢复态。这个 Renderer 协调层不新增 IPC、schema 或 migration。
- 标题最多 200 个 Unicode code point；Markdown 正文最多 100,000 个 code point。Main 会把 CRLF/CR 统一为 LF，保留 Markdown、代码和普通换行，同时拒绝 NUL、畸形 Unicode 和不支持的控制字符。
- 编辑与归档必须提交 `expectedRevision`。数据库中的 revision 已变化时拒绝旧写入，Renderer 需要基于最新快照重新处理，不能让迟到自动保存覆盖新内容。
- 归档是软归档。归档工作区中的笔记继续进入备份，但 Service 与 Trigger 都拒绝修改或删除。
- “笔记”分类仍只是收件箱处理意图。显式转换会在一个事务中归档来源、创建真实笔记并建立唯一来源关系，响应中的 `createdNoteId` 是该事务实际插入的笔记；同一来源不能同时转换为任务和笔记。
- Markdown 内容始终按不可信文本处理。预览只通过受控 React 节点生成，不执行原始 HTML，也不把 `javascript:`、`data:` 或本地文件链接带入可信 Renderer。

## 滚动 7 日日程

日程快照包含目标工作区、Main 计算的本地 `todayDate`、同一份固定 7 日 `{token, date}` 窗口和窗口内活动项。不包含重复规则、提醒、系统通知或外部日历同步。

- 日程 ID、持久化日期、时间戳和 revision 由 Main 控制；Renderer 只能提交 `expectedDate`、标题、固定类型以及分钟范围。
- 既有 `schedule:create` 在同一个 Main 事务中返回事务后的完整 `scheduleSnapshot` 与实际插入的 opaque `createdScheduleId`；这只扩展返回契约，不新增 IPC 通道、schema 或 migration。Today 与 7 日计划创建成功后仍留在 Today；只有用户显式打开时才 fresh-read 当前工作区日程、唯一匹配该 ID 与持久化日期，并用权威快照中的当前 revision 打开编辑窗口。目标缺失、重复 ID、跨午夜、切页、较新创建、工作区 A→B→A 或迟到请求都失败关闭，不按标题、类型、时段或列表位置回退。
- `schedule:update` 与 `schedule:archive` 的 Main 成功是不可重放的提交边界。Renderer 冻结原行与目标字段，更新只接受同工作区、同持久化日期、精确 ID、创建时间、规范化内容/时段和预期 revision 的唯一行；归档只接受覆盖目标日期的权威快照中精确 ID 缺失。规范化后无变化的更新保持原 revision 与 `updatedAt`。响应快照若未能发布，Controller 会复查 ref-backed 最新快照并最多准备两次权威重读；跨午夜恢复必须先有可信 Main 响应中的精确结果，不能仅凭旧日期从新窗口消失推断成功。
- Main 已提交但 Renderer 仍无法确认时，App 保留跨页面的失败关闭警告和精确 mutation 身份。显式“重新读取”只运行同一对账，不会再次调用 update/archive；恢复完成前，同工作区创建、编辑、归档、工作区切换和日程搜索跳转都被阻止，相关日程入口同步禁用。取消或失败的数据导入/备份恢复保留警告，只有成功替换数据库后才使它失效；焦点在警告动作、错误与原日程/页面标题之间受控交接。
- 若 Main 已提交创建但 Renderer 在有界重读后仍无法提交权威快照，创建窗口会关闭并显示独立警告，明确要求刷新且不要重复创建；此路径不发布未经确认的成功回执。
- 类型固定为 `focus`、`meeting`、`review`、`personal`。时间使用本地民用日期和 `0..1440` 分钟边界，结束分钟必须晚于开始分钟，不依赖 locale 字符串或 UTC 日期切片。
- 创建、编辑和归档都会验证 `expectedDate` 位于 Main 执行时计算的 `[todayDate, todayDate + 6]` 民用日期窗口；过去日期和第 8 天都会拒绝，跨午夜后旧页面不能把过期请求写入新窗口。
- 编辑与归档使用 `expectedRevision` 防止迟到响应覆盖新状态。持久化日期不可修改；如需另一天，用户显式归档旧项并在目标日期新建。
- Renderer 在本地午夜、窗口重新聚焦和页面重新可见时同步丢弃旧日快照并重新读取；任务、日程或专注任一层仍持有旧日期时，不会与新窗口组合绘制。
- 归档工作区保留日程历史以进入一致性备份，但不再允许修改或删除。

## 浏览器标签与收藏夹

浏览器持久数据与既有业务模块共用数据库单例和 FIFO 操作队列。首次读取活动工作区时，Main 会在一个事务内初始化默认标签和活动状态；Renderer 不提供 ID、时间戳、revision 或页面标题。

- 标签、活动标签状态和收藏夹严格绑定 `workspaceId`。所有 mutation 返回完整持久快照，Main Controller 再合并当前远程页面的前进/后退、加载状态和仅运行时下载。
- 浏览器状态使用精确 `revision + 1` 触发器。关闭最后一个标签会原子重置为默认页，保证活动工作区始终至少有一个标签和有效活动引用。
- 标签导航完成后的 URL 与标题按每标签合并到单一有界写入 worker，连续事件只持久化最新期望值；显式导航从 Main 接受后会保留 pending URL，直到真实导航成功或失败，切换/退出期间不会用仍未提交的旧 `getURL()` 反写。关闭、收藏、工作区切换或受控退出前会等待同一标签的元数据落盘，避免写放大、收藏旧 URL、删除后迟到写入或关窗丢失最终状态。
- 收藏夹只接受当前受管标签的安全 HTTP(S) URL；同一工作区 URL 唯一。归档工作区中的标签、状态和收藏夹保留进备份，但 Service 与数据库触发器拒绝继续修改或删除。
- SQLite 不保存网页会话历史、Cookie、登录态或下载记录；这些运行时信息不会伪装成工作区持久数据。

## 打包

Electron Forge 负责启动、原生模块 rebuild 与平台打包；Vite 分别构建 Main、Preload 和 Renderer。通过项目 npm scripts 启动或打包前，会清理明确的 `.vite` 与 `out` 目录，防止旧入口或旧安装包混入产物。`node-pty` 保持外部依赖，并由 auto-unpack-natives 从 ASAR 解包。

应用启用 Electron fuses，关闭 Node CLI 参数、`NODE_OPTIONS` 和浏览器专用 V8 snapshot 等非必要入口。Windows ConPTY 的关闭流程依赖 `child_process.fork`，因此必须保留 RunAsNode。可信 Renderer 当前通过 `file://` 加载，所以暂时显式保留该协议的额外权限；迁移到自定义 `app://` 协议后再关闭。

Electron 43 有 9 项 V1 fuse，而 Forge 7 当前兼容的 `@electron/fuses` 1.x 只能命名前 8 项。打包验证会直接断言 wire 长度和全部 9 项状态（包括 `WasmTrapHandlers`）；Electron 新增 fuse 或任一状态漂移都会使 CI 失败。Windows CI 还会对 Squirrel 构建同时产生的未打包应用负载执行原生文件检查、多终端生产管理器，以及 v11 工作区、收件箱、任务、笔记、日程、专注与浏览器持久数据、v0–v10→v11 迁移、工作区恢复、FTS 回填、终端偏好、自动化、备份与 `.dwbx` v1/v2/v3 冒烟；ASAR 门禁同时检查浏览器、下载、终端、专注、自动化与 AI IPC 安全入口和已移除旧协议，成功后才生成校验和并上传安装制品。最终 NUPKG 负载复验仍由独立的 Issue #9 跟踪。

下载运行时另有独立 Vite 入口，只导入生产 `DownloadManager`，不会给产品 Main 增加测试开关。Linux 在 Xvfb、Windows 在固定的 Electron 43.2.0 可执行文件中，用本地 HTTP Range 服务和真实 `WebContentsView` 用户手势验证 `will-download`、SHA-256、暂停/恢复、取消、非受管/无手势拒绝及定位回调；ASAR token 门禁继续负责证明相同安全入口已进入正式包。

终端运行时也有独立 Vite 入口，只导入生产 `TerminalManager`、配置服务、Profile resolver 与受控 WSL discovery。Linux 与 Windows 都在正式打包负载中验证固定 Profile、带空格与 Unicode 的本机 CWD、配置 revision、两个并行会话、冻结启动描述、独立输出、resize、clear、关闭一个后另一个继续工作，以及 A→B→A 工作区隔离；Windows 只有在机器真实存在 WSL 发行版时才执行绑定所选发行版并从 Linux home 启动的断言，否则必须返回受控不可用状态。

AI provider 另有独立 Vite 入口，只导入生产 Responses provider 和测试专用的 loopback SSE server。Linux 与 Windows 都在正式打包负载中以假 key 断言生产 provider 仍调用固定 OpenAI URL，再由测试专用的注入 `fetch` 实现把该请求转送到 loopback SSE double；由此验证固定 model、`store: false`、空工具列表、任意网络分块下的文本增量、完成、取消、HTTP 错误与断流，而不会读取用户凭据、访问真实 OpenAI endpoint 或产生 API 费用。产品和测试入口都不存在 endpoint override，测试入口也不会进入 ASAR。

原生模块不能跨系统复用。Windows 安装包必须在 Windows x64 环境执行 `npm ci` 与 `npm run make`，并在升级 Electron 或 `node-pty` 后验证打包产物中的真实终端。

完整依赖树与生产依赖采用两层门禁：生产依赖不允许任何等级的已知漏洞；Forge 尚未修复的开发期链路按 GHSA 设置有期限例外，任何新漏洞、例外过期或风险进入生产树都会阻断。详见[依赖风险说明](DEPENDENCY_RISKS.md)。

## 业务数据边界

业务 Repository 会继续放在 Main，Renderer 仍只能经 preload 使用业务 API：

```text
Workspace
├─ Inbox ✓
├─ Tasks ✓
├─ Notes ✓
├─ Schedule ✓ (rolling 7-day window)
├─ Focus sessions ✓ (persistent, app-runtime scheduler)
├─ Browser tabs/bookmarks ✓
├─ Terminal sessions ✓ (runtime)
├─ Terminal profile/CWD/WSL preferences ✓ (machine-local)
├─ Scheduled automations ✓ (app-runtime scheduler)
├─ AI assistant ✓ (runtime, explicit bounded context)
├─ Global search/quick actions ✓
├─ Backup policy/data portability ✓
└─ Layout state ✓
```

后续迁移和数据管理扩展仍需沿用同一边界，避免页面直接依赖表结构或文件路径。
