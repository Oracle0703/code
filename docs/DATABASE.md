...
工作区或收件箱事务如果无法可靠回滚，数据库服务会立即进入 fatal 状态：当前错误向上返回，已排队及后续读取、写入和备份全部拒绝，直到连接关闭并由新实例重新打开。应用不会在事务结果未知的连接上继续服务。
...
收件箱 v3 不导入界面中的演示条目，也不会把原型 `localStorage` 任务当作真实收件箱数据。条目允许重复正文，分类只接受 `uncategorized`、`task`、`note`、`link`；其中 `task` 是后续处理意图，不创建任务表或无外键约束的多态关联。归档工作区中的条目原样保留，但触发器和 Service 都拒绝继续修改。

## 备份与未来恢复

备份使用 SQLite online backup API 写入同目录的唯一临时文件，验证快照可只读打开且通过迁移、工作区和全部收件箱行映射后，再原子重命名为最终文件。列表会忽略临时文件、旁路文件和符号链接；Renderer 得到的是序列化元数据，不是绝对路径。
...
- 普通 Node 测试：新建、v1/v2 升级、迁移回滚、工作区/收件箱事务、撤销令牌、损坏拒绝、备份一致性和关闭队列。
- Linux 打包测试：真实 Electron 可执行文件中的 `node:sqlite`、WAL、v1/v2→v3、工作区、收件箱、备份和重开。
- Windows 打包测试：同一数据库、工作区与收件箱冒烟链路在 Squirrel 产物对应的 Electron/SQLite 运行时中通过。
