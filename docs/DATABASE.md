...
Repository 负责 SQL 与严格行映射，Service 负责生命周期、串行化、事务、不变量和对外返回值。业务页面只能通过 `Renderer → Preload → 可信 IPC → Service → Repository` 接入。数据库元数据与工作区、收件箱、任务、笔记、日程启动校验在同一事务提交，校验失败不会留下已更新的 `last_opened_at` 半状态。

工作区、收件箱、任务、笔记或日程事务如果无法可靠回滚，数据库服务会立即进入 fatal 状态：当前错误向上返回，已排队及后续读取、写入和备份全部拒绝，直到连接关闭并由新实例重新打开。应用不会在事务结果未知的连接上继续服务。
...
备份使用 SQLite online backup API 写入同目录的唯一临时文件，验证快照可只读打开且通过迁移、工作区以及对应 schema 版本的全部收件箱、任务、笔记和日程行映射后，再原子重命名为最终文件。列表会忽略临时文件、旁路文件和符号链接；Renderer 得到的是序列化元数据，不是绝对路径。
...
- 普通 Node 测试：新建、v1/v2/v3/v4 升级、迁移回滚、工作区/收件箱/任务/笔记/日程事务、撤销令牌、任务与笔记转换、revision 冲突、损坏拒绝、备份一致性和关闭队列。
- Linux 打包测试：真实 Electron 可执行文件中的 `node:sqlite`、WAL、新库与 v1/v2/v3/v4→v5、工作区、收件箱、任务、笔记、日程、两类转换、备份和重开。
- Windows 打包测试：同一数据库、工作区、收件箱、任务、笔记与日程冒烟链路在 Squirrel 构建产生的未打包应用负载所对应的 Electron/SQLite 运行时中通过。最终 NUPKG 负载复验由独立的 Issue #9 跟踪。
