...
> 当前状态：`v0.7` 搜索与数据保护闭环。Electron 进程隔离、可迁移数据库、浏览器标签/收藏夹/安全下载、工作区隔离的多终端，以及全局搜索与受控数据可移植性已经打通。
...
- 命令面板中的工作区切换、页面导航、快速记录、浏览器、终端和数据设置快捷动作
...
- Main 独占的 `.dwbx` 逻辑导出/预检导入与崩溃可恢复替换，不接受 Renderer 路径或外部 SQLite
- Linux/Windows 打包后 SQLite、工作区、收件箱、任务、笔记、日程、浏览器、搜索、迁移、备份和重开验证
...
1. 可恢复的终端 Profile/CWD 偏好与受控 WSL 配置
2. 定时自动化与 Codex/AI 能力接入
