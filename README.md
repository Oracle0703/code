...
> 当前状态：归档工作区安全恢复闭环。Electron 进程隔离、schema v11 可迁移数据库、浏览器标签/收藏夹/安全下载、工作区隔离的多终端、滚动 7 日计划、全局搜索、受控数据可移植性、每日/每周固定自动化、只读工作区 AI 助手与可恢复专注会话已经打通；归档项现在可通过 Main 签发 revision 安全恢复，且不会自动切换或复活已释放运行时。
...
| 目标            | 当前验证级别                                                                     |
| --------------- | -------------------------------------------------------------------------------- |
| Windows x64     | Squirrel、fuse、ConPTY、v0–v10→v11/工作区恢复、真实下载及本地 AI provider 冒烟   |
| Linux x64       | Electron package、fuse、包体、v0–v10→v11/工作区恢复、真实下载及 AI provider 冒烟 |
