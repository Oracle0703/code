...
> 当前状态：`v0.4` 日常工作闭环。Electron 进程隔离、真实工具面板、可迁移数据库、工作区、收件箱、任务、Markdown 笔记与今日日程已经打通。
...
- 受控手动备份 API，以及 Linux/Windows 打包后 SQLite、工作区、收件箱、任务、笔记、日程、迁移、备份和重开验证
...
1. 浏览器多标签、收藏夹和下载管理
2. PowerShell、CMD、WSL 多终端配置
3. 全局搜索、快捷动作、导入导出和定时备份
4. 定时自动化与 Codex/AI 能力接入
