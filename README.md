...
> 当前状态：滚动 7 日计划闭环。Electron 进程隔离、可迁移数据库、浏览器标签/收藏夹/安全下载、工作区隔离的多终端、全局搜索、受控数据可移植性、每日/每周固定自动化、只读工作区 AI 助手与可恢复专注会话已经打通；Today 现在还能在同一份 Main 日期窗口中安排接下来 7 天的任务与日程。
