...
describe('DatabaseService', () => {
  ...
  it('creates an opaque, validated manual snapshot in the controlled backup directory', async () => {
    ...
    expect(backup).toMatchObject({ reason: 'manual', schemaVersion: 11 });
    expect(backup.fileName).toMatch(/^daily-workbench-v11-manual-.+\.sqlite3$/u);
    ...
    try {
      expect(snapshot.prepare('PRAGMA user_version').get()).toEqual({ user_version: 11 });
  ...
  it('still initializes inside a pre-created empty data directory without legacy evidence', async () => {
    ...
    await expect(fresh.open()).resolves.toMatchObject({
      migration: { fromVersion: 0, toVersion: 11 },
    });
    await fresh.close();
  });
  ...
  it('promotes a committed first database when the process stopped before publishing its sentinel', async () => {
    ...
    const resumed = new DatabaseService({ dataDirectory });
    await expect(resumed.open()).resolves.toMatchObject({
      migration: { fromVersion: 11, toVersion: 11 },
    });
  ...
  it('backs up an existing schema before applying a newly appended migration', async () => {
    ...
    expect(result.migration).toMatchObject({ fromVersion: 11, toVersion: 12 });
    ...
    expect(backups[0]).toMatchObject({ reason: 'pre-migration', schemaVersion: 11 });
  ...
  it('backs up v7 before current migrations and backfills terminal defaults without changing the old snapshot', async () => {
    const dataDirectory = await createDataDirectory();
    const workspaceId = '11111111-1111-4111-8111-111111111111';
    const versionSeven = new DatabaseService({
      dataDirectory,
      migrations: DEFAULT_MIGRATIONS.slice(0, 7),
      workspaceIdFactory: () => workspaceId,
    });
    await expect(versionSeven.open()).resolves.toMatchObject({
    ...
    const upgraded = new DatabaseService({ dataDirectory });
    const result = await upgraded.open();
    expect(result.migration).toMatchObject({ fromVersion: 7, toVersion: 11 });
    expect(result.preMigrationBackup).toMatchObject({
      reason: 'pre-migration',
      schemaVersion: 7,
    });
    await expect(upgraded.getTerminalPreferences(workspaceId)).resolves.toMatchObject({
      workspaceId,
