...
describe('DatabaseService', () => {
  ...
  it('creates an opaque, validated manual snapshot in the controlled backup directory', async () => {
    ...
    expect(backup).toMatchObject({ reason: 'manual', schemaVersion: 4 });
    expect(backup.fileName).toMatch(/^daily-workbench-v4-manual-.+\.sqlite3$/u);
  ...
  it('backs up an existing schema before applying a newly appended migration', async () => {
    ...
    expect(result.migration).toMatchObject({ fromVersion: 4, toVersion: 5 });
    ...
    expect(backups[0]).toMatchObject({ reason: 'pre-migration', schemaVersion: 4 });
