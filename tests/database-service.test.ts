...
describe('DatabaseService', () => {
  ...
  it('creates an opaque, validated manual snapshot in the controlled backup directory', async () => {
    ...
    expect(backup).toMatchObject({ reason: 'manual', schemaVersion: 3 });
    expect(backup.fileName).toMatch(/^daily-workbench-v3-manual-.+\.sqlite3$/u);
  ...
  it('backs up an existing schema before applying a newly appended migration', async () => {
    ...
    expect(result.migration).toMatchObject({ fromVersion: 3, toVersion: 4 });
    ...
    expect(backups[0]).toMatchObject({ reason: 'pre-migration', schemaVersion: 3 });
